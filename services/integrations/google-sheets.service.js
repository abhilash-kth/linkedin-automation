...
export async function setupLeadsSheetHeaders() {
  ...
  if (result) {
    console.log(`✅ Headers set: A (${SHEET_HEADERS[0]}) → AT (${SHEET_HEADERS[45]})`);
  }
  return result;
}
...
export function buildLeadRow(lead) {
  ...
  const commentDate =
    lead.status === "commented" && lead.lastProcessedAt
  ...
  const connectionStatus =
    ["accepted", "message_sent", "replied", "interested"].includes(lead.status)
      ? "accepted"
      : ["connection_sent", "connection_and_message_sent", "pending_acceptance"].includes(lead.status)
      ? "pending"
      : "";

  const replied = ["replied", "interested", "meeting_scheduled"].includes(lead.status);
  ...
  row[15] = lead.status === "commented" ? "Yes" : "No";  // P  Comment Posted
